...
- **📁 Advanced File Browser & Storage:** Built-in clean file browser with quick navigation. Files are conveniently saved straight to your device's native `Downloads/ClipLAN` directory so you never lose track of them.
- **📋 Network Clipboard:** Seamlessly share text snippets and URLs across your local network in real-time.
- **🔄 True Background Transfers:** You can close the app or lock your phone—transfers will continue seamlessly in the background with a persistent notification (Android) using foreground services.
...
2. **Transfer Engine**: Files are broken down and streamed in memory-efficient 8MB chunks over raw TCP sockets (port 53318), completely saturating your internal storage IO and Wi-Fi band.
